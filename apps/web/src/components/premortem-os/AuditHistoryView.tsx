...
export function AuditHistoryView({ audits, onFetchAuditDetail, onSelectAudit, setActiveTab }: AuditHistoryViewProps) {
  ...
  const [comparisonAudits, setComparisonAudits] = useState<{ runA?: AuditRun; runB?: AuditRun }>({});
  ...
  const runA = comparisonAudits.runA ?? audits.find(a => a.id === comparisonRunAId);
  const runB = comparisonAudits.runB ?? audits.find(a => a.id === comparisonRunBId);
        ...
        <div className="bg-[#FAF8F5] border border-[#EAE6DF] rounded-lg p-6 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-4 pb-1 text-[10px] font-mono text-[#5C6560]">
            <span className="inline-flex items-center gap-1.5">
              <span className="inline-block h-0.5 w-4 bg-[#064E3B]" aria-hidden />
              Compliance Rating (0–100)
            </span>
          </div>

          <div className="relative z-10 h-60 w-full pt-4">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 10, right: 30, left: 0, bottom: 24 }}>
                ...
                <Tooltip
                  ...
                  cursor={{ stroke: '#064E3B', strokeWidth: 1, strokeDasharray: '4 4' }}
                />
                <Line 
                  type="monotone" 
                  dataKey="score" 
                  stroke="#064E3B" 
                  strokeWidth={2.5}
                  activeDot={{ r: 6, fill: '#064E3B', stroke: '#FAF8F5', strokeWidth: 2 }} 
      ...
      <div className="p-6 bg-white border border-[#EAE6DF] rounded-lg space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 items-end text-xs">
          ...
          <div>
            ...
            >
              <RotateCw size={12} className={`shrink-0 ${isComparing ? 'animate-spin' : ''}`} />
              <span>{isComparing ? 'Loading Snapshots…' : 'Execute Comparative Trace'}</span>
            </button>
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start text-xs">
              ...
              <div className="border border-rose-200 bg-rose-50/5 rounded-lg overflow-hidden flex flex-col h-full min-h-[300px]">
                <div className="p-4 bg-rose-950 text-white font-mono flex items-center justify-between shrink-0 font-bold uppercase text-[9.5px]">
                  ...
                  <span className="bg-rose-900 border border-rose-800 text-[8.5px] px-1.5 py-0.2 rounded text-rose-300 font-bold animate-pulse">ATTENTION REQUIRED</span>
                </div>
                ...
                <div className="flex-1 p-4 overflow-y-auto space-y-3">
                  {comparisonResults.newVulnerabilitiesIntroduced.length === 0 ? (
                    ...
                    comparisonResults.newVulnerabilitiesIntroduced.map((v, idx) => (
                      <div key={idx} className="border border-rose-200 bg-white p-3.5 rounded shadow-xs space-y-1.5 relative overflow-hidden group">
                        <div className="flex justify-between items-center font-mono text-[9px]">
                          <span className={`${getSeverityStyle(v.severity)} font-bold px-1.5 border py-0.2 rounded`}>{v.severity}</span>
                          <span className="text-zinc-400 select-text bg-zinc-50 border px-1 rounded">{v.filepath}:{v.line}</span>
                        </div>
