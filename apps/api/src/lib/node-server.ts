...
import { ensureLocalDevelopmentFixture } from '@premortem/db/dev-fixtures';
...
export async function startPremortemNodeServer(options: {
  serviceName: string;
  healthService: string;
  host?: string;
  port?: number;
  seedLocalFixture?: boolean;
}) {
  await initServerObservability(options.serviceName);

  if (options.seedLocalFixture && isLocalAuthBypassEnabled()) {
  ...
  const host = options.host ?? (options.seedLocalFixture ? '127.0.0.1' : '0.0.0.0');
  const port = options.port ?? Number.parseInt(process.env.PREMORTEM_API_PORT ?? process.env.PORT ?? '18787', 10);
