...
const headerStyle = {
  ...
  display: 'grid',
  gap: 18,
  background: 'linear-gradient(135deg, rgba(255,252,244,0.97), rgba(255,239,201,0.93))'
} as const;
...
const statusPillStyle = (completed: boolean) =>
  ({
    alignSelf: 'start',
    padding: '10px 14px',
    borderRadius: 999,
    background: completed ? '#dff7df' : '#fce4d6',
    color: completed ? '#196c2e' : '#8d2f18',
    fontWeight: 700,
      ...
      <section style={sectionStyle}>
        <header style={headerStyle}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
            ...
            <div style={statusPillStyle(auditRun.runStatus === 'completed')}>
              {auditRun.runStatus}
            </div>
          </div>
          ...
          {auditRun.rejectedIssueCandidates.length === 0 ? (
            ...
            <div style={{ display: 'grid', gap: 12 }}>
              {auditRun.rejectedIssueCandidates.map((issue: AuditRunSnapshot['rejectedIssueCandidates'][number]) => (
                <article key={issue.id} style={rejectedCardStyle}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
