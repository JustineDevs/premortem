...
function fromUrl(url) {
  ...
  return serialized.replace(/^postgres:/i, 'postgresql:');
}
...
export function shouldNormalizeSupabaseDatabaseUrl(raw) {
  const url = toUrl(raw);
 ...
 */
export function normalizeTransactionPoolerUrl(raw, env = process.env) {
  const url = toUrl(raw);
 ...
 */
export function normalizeSessionPoolerUrl(raw, transactionUrl) {
  let url = toUrl(raw);
  ...
  if (isDirectSupabaseHost(url.hostname)) {
    ...
    const tx = toUrl(transactionUrl);
    url = new URL(tx.toString());
  }
...
export function deriveSessionPoolerFromTransaction(transactionUrl) {
...
export function applySupabaseDatabaseEnv(env = process.env) {
