...
async function main() {
  ...
  console.log(
    `Env: GITLAB_TOKEN=${process.env.GITLAB_TOKEN ? "set" : "missing"}, DATABASE_URL=${process.env.DATABASE_URL ? "set" : "missing"}, PHOENIX_API_KEY=${process.env.PHOENIX_API_KEY ? "set" : "missing"}\n`
  );
