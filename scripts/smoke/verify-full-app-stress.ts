...
const securityModule = await import('@premortem/security');
...
const {
  buildWorkerRegisteredAgents,
  executeAuditJob,
  getAuditRunSnapshot,
  submitAudit
} = orchestratorModule;
const { identityHeadersForActorContext } = securityModule;
...
async function expectRedirect(pathname, label, allowed = [302, 303, 307, 308]) {
  const response = await fetch(`${WEB_BASE}${pathname}`, { redirect: 'manual' });
  if (!allowed.includes(response.status)) {
    fail(label, `${pathname} status ${response.status}`);
    return;
...
/** @type {string | null} */
  ...
  await expectRedirect('/reviews', 'legacy /reviews → /app', [307, 308, 302, 303]);

  const appResponse = await fetch(`${WEB_BASE}/app`, { redirect: 'manual' });
  if (appResponse.status === 200) {
    pass('console /app renders');
  } else if ([302, 303, 307, 308].includes(appResponse.status)) {
...
for (const auditRunId of parallelAudits) {
  ...
  if (snapshot?.graphSnapshot?.nodeCount > 0) {
...
const snapshotPayload = lastAuditId ? await getAuditRunSnapshot(lastAuditId) : null;
const issueId = snapshotPayload?.issueCandidates?.[0]?.id;
const issueCandidate = snapshotPayload?.issueCandidates?.[0];
  ...
  if (publishPayload?.dryRun || publishPayload?.ok) {
    ...
    skip('publish', 'no issue candidate available in smoke fixture');
  }
...
const sandboxPayload = await expectJson('POST', '/api/audits/run', 'sandbox scan', {
  ...
  if (sandboxPayload?.success) {
    ...
    sandboxPayload?.code === 'repo_limit' ||
    (typeof sandboxPayload?.error === 'string' && sandboxPayload.error.includes('repo_limit'))
  ) {
    skip('sandbox', 'project limit reached in smoke fixture');
  } else {
...
void main().catch((error) => {
  console.error('verify-full-app-stress failed', error);
  process.exitCode = 1;
});
