...
import { ensureDockerServices } from '../docker/ensure-services.ts';
import { assertNeo4jReachable, assertProductionSmokePrerequisites, loadSmokeEnv } from './load-smoke-env.ts';
import {
  createSupabaseSmokeSession,
  deleteSupabaseSmokeUser
} from './smoke-supabase-session.ts';
...
} = await import('@premortem/db/dev-fixtures');
const {
...
if (process.env.SENTRY_DSN) {
  await probeSentryDelivery('premortem-production-readiness', 'premortem-production-readiness');
  pass('observability', 'Sentry delivery verified');
} else {
...
if (process.env.NEXT_PUBLIC_POSTHOG_KEY || process.env.POSTHOG_API_KEY) {
  await probePostHogDelivery(
    `premortem-production-readiness-${Date.now().toString(36)}`,
    'premortem_production_readiness'
  );
  pass('observability', 'PostHog delivery verified');
} else {
...
if (process.env.LANGFUSE_PUBLIC_KEY && process.env.LANGFUSE_SECRET_KEY) {
  await probeLangfuseDelivery({
    name: 'premortem.production_readiness',
    comment: 'Production readiness smoke probe'
  });
  pass('observability', 'Langfuse delivery verified');
} else {
  skip('observability', 'Langfuse configured', 'LANGFUSE_PUBLIC_KEY / LANGFUSE_SECRET_KEY unset locally');
}
