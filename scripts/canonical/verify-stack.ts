 ...
 * Complements scripts/mcp/verify-all.ts (MCP connectivity).
 */
...
const checks = [
  ...
  {
    name: 'Alibaba Cloud ECS deployment helper',
    status: () =>
      existsSync(resolve(ROOT, 'scripts/deploy/alibaba-cloud-ecs.ts')) ? 'OK' : 'FAIL',
    detail: 'scripts/deploy/alibaba-cloud-ecs.ts: backend deployment proof helper'
  },
  {
    name: 'TanStack Query',
    status: () =>
...
console.log('  - Run: node scripts/mcp/verify-all.ts');
