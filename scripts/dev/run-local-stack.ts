...
async function main() {
  ...
  const { applyConfiguredDevDefaults } = await import(
    pathToFileURL(path.join(repoRoot, 'packages/domain/dist/index.js')).href
  );
  ...
  console.log(
    JSON.stringify({
      ...
      mode: useFixtureDefaults ? 'fixture' : hasRealLlm ? 'configured' : 'production',
